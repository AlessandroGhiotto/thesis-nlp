...
- **`_adaptiveICL/`**  
  Everything related to AdaptiveICL from Manish Chandra. Train a model for predicting the number of k examples to be used in a k-shot prompting task.
...
### `_adaptiveICL/` Example Usage

- Example command to run FICL baseline
...
- Example command to run AICL
  1. Example command to generate data for training the k-predictor model
  ```
  python -m src.main --prepare --dataset sst2 --method dynamic --model_name microsoft/phi-2 --oracle_split train
  ```
  2. Example command to run AICL
  ```
  python -m src.main --dataset sst2 --method dynamic --model_name microsoft/phi-2
  ```

Credits: Manish Chandra's [AdaptiveICL](https://github.com/ManishChandra12/adaptiveICL/tree/master)
