...
def mappinglabels(train):
    labels = [label for label in train['train'].features.keys() if label not in ['id', 'txt','typet']]
...
def preprocess_data(train,dev):
    ...
    tokenized_train = train.map(tokenizedata, batched=True, remove_columns=train['train'].column_names)
    tokenized_dev = dev.map(tokenizedata, batched=True, remove_columns=dev['train'].column_names)
...
def classification(tokenized_train, tokenized_dev, id2label, label2id, modelname, numlabels):
    ...
    model = AutoModelForSequenceClassification.from_pretrained("roberta-large", problem_type="multi_label_classification", num_labels=numlabels,id2label=id2label, label2id=label2id).to(device)
...
def multi_label_metrics(predictions, labels, threshold=0.2):
    ...
    metrics = {'f1': f1_micro_average,'roc_auc': roc_auc,'accuracy': accuracy,'f1-macro':f1_macro_average}
    return metrics
...
def predictMultilable(test):
    ...
    allpredictions = []
    allactual = []
    model = AutoModelForSequenceClassification.from_pretrained('/home/aleks/PycharmProjects/pythonProject/RobertaFiles/toxicLarge/checkpoint-2216')
    tokenizer = AutoTokenizer.from_pretrained('/home/aleks/PycharmProjects/pythonProject/RobertaFiles/toxicLarge/checkpoint-2216', use_fast=True)
    ...
    f1_micro_average = f1_score(y_true=allactual, y_pred=allpredictions, average='micro')
    f1_macro_average = f1_score(y_true=allactual, y_pred=allpredictions, average='macro')
