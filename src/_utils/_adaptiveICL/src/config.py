def get_config(args):
    ...
    if dataset == "sst2":
        ...
        classes = [
            ...
            "Location",
            "Number",
        ]  # ['Acronym', 'Body', 'Narration', 'Person', 'Place', 'Figure']
        ...
        prompt_suffix = (
            "Question: Does the premise entail the hypothesis, true or false? Answer: "
        )
        batch_size = 3
        eval_steps = 50
        learning_rate = 5e-3
    ...
    ### ! ADDED AGNEWS HERE
        ...
        classes_in_data = ["World", "Sports", "Business", "Sci/Tech"]
        if K_max == 0:
            prompt_prefix = "For the given sentence, your task is to classify it into one of the given categories. Choose one among world, sports, business and science/technology.\n"
        else:
            prompt_prefix = "For the given sentence, your task is to classify it into one of the given categories. Choose one among world, sports, business and science/technology. Here are a few examples:\n"
        prompt_suffix = "Class: "
        batch_size = 10
        eval_steps = 50
        learning_rate = 5e-3
    ...
    if prepare or method == "dynamic":
        ...
        embed = True
        neigh = True
        input_dim = embed * 384 + neigh * (
            1 + K_max
        )  # 1 #(len(classes) * K_max) #(1+K_max)#(1+len(classes)) #len(classes) #K_max
        output_dim = K_max + 1
    return (
        train_datapath,
        test_datapath,
