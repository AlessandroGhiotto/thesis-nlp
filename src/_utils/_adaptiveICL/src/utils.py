...
def arg_parser():
    ...
    parser.add_argument(
        "--save_preds", action="store_true", help="Save final predictions"
    )
    ...
    parser.add_argument(
        "--prepare",
        action="store_true",
        help="Prepare data for training k-predictor model",
    )
    ...
    parser.add_argument(
        ...
        help="How to select demonstrations",
        default="similar",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="test",
        help="Use dev to identify best k and test for final evaluation",
        required=False,
    )
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument(
        "--no_single_precision", dest="subset_exists", action="store_false"
    )
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        help="Cache dir for storing transformers models",
        default="/scratch/manish/hf_cache/",
        required=False,
    )
    parser.add_argument("--labels_available", action="store_true")
    parser.add_argument(
        "--no_labels_available", dest="labels_available", action="store_false"
    )
...
def get_kdtree(dataset):
    ...
    train_sentences = [
        ex.text_a if ex.text_b == "" else ex.text_a + " " + ex.text_b
...
def load_model_tokenizer(model_name, single_precision, cachedir):
    quantization_config = BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_compute_dtype=torch.float16 if single_precision else torch.float32,
    )
    if (
        model_name == "meta-llama/llama-2-7b-hf"
        or model_name == "meta-llama/llama-2-13b-hf"
    ):
        ...
        tokenizer = LlamaTokenizer.from_pretrained(
            model_name, padding_side="left", cache_dir=cachedir
        )
    elif model_name in ["microsoft/phi-2", "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"]:
        ...
        with open(path, encoding="utf-8") as f:
            ...
            for idx, line in enumerate(lines[0:]):
                ...
                example = InputExample(
                    guid=guid, text_a=text_a, label=self.label_mapping[label]
                )
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, "rb") as f:
            for idx, line in enumerate(f):
                ...
                example = InputExample(
                    guid=guid, text_a=text, label=self.label_mapping[coarse_label]
                )
                examples.append(example)
        return examples
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, "r") as f:
            for idx, line in enumerate(f):
                ...
                guid = "%s-%s" % (split, idx)
                example = InputExample(
                    guid=guid, text_a=text, label=self.label_mapping[label]
                )
                examples.append(example)
        return examples
...
### ! ADDITION HERE
class csvProcessor:
    """A processor for csv files with two columns: text and label."""
...
def load_dataset(dataset, train_datapath, test_datapath, classes_in_data, tokenizer):
    dataset_dict = dict()
    if dataset == "sst2":
        dataset_dict["train"] = SST2Processor(classes_in_data).get_examples(
            train_datapath, "train"
        )
        dataset_dict["test"] = SST2Processor(classes_in_data).get_examples(
            test_datapath, "test"
        )
        dataset_dict["dev"] = SST2Processor(classes_in_data).get_examples(
            test_datapath, "dev"
        )
    elif dataset == "trec":
        train_val = TrecProcessor(classes_in_data).get_examples(train_datapath, "train")
        dataset_dict["test"] = TrecProcessor(classes_in_data).get_examples(
            test_datapath, "test"
        )
        dataset_dict["train"], dataset_dict["dev"] = train_test_split(
        ...
        dataset_dict["test"] = colaProcessor(classes_in_data).get_examples(
            test_datapath, "dev"
        )
        dataset_dict["train"], dataset_dict["dev"] = train_test_split(
            train_val, test_size=0.1, random_state=42, shuffle=True
        ...
        dataset_dict["test"] = rteProcessor(classes_in_data).get_examples(
            test_datapath, "dev"
        )
        dataset_dict["train"], dataset_dict["dev"] = train_test_split(
            train_val, test_size=0.1, random_state=42, shuffle=True
    ...
    for h in dataset_dict["train"]:
        if h.label in train_labels.keys():
            ...
            train_labels[h.label] = 1
    print("Train labels count after splitting:", dict(sorted(train_labels.items())))
    dev_labels = dict()
    for h in dataset_dict["dev"]:
        if h.label in dev_labels.keys():
            ...
            dev_labels[h.label] = 1
    print("Dev labels count after splitting  :", dict(sorted(dev_labels.items())))
    print(
        "Id to label mapping               :",
        dict(sorted({i: k for (i, k) in enumerate(classes_in_data)}.items())),
    )
...
def pred_batch(
    ...
    if k == 0:
        ...
        if mode == "random":
            K_examples_all = [
                random.sample(dataset_dict["train"], k) for _ in range(batch_size)
            ]
        elif mode == "similar":
            test_sentences = [
                (
                    f"{test_example.text_a}"
                    if test_example.text_b == ""
                    else f'{test_example.text_a + " " + test_example.text_b}'
                )
                for test_example in dataset_dict[splt][indexes]
            ]
            test_embeddings = sbert_model.encode(
                test_sentences
            )  # Get the embedding of the test_sentence
            if prepare and splt == "train":
                _, top_k_indices = tree.query(
                    test_embeddings, k=k + 1
        ...
        if dataset == "sst2":
            enc = tokenizer.batch_encode_plus(
                [
                    ...
                    for indx, test_example in enumerate(dataset_dict[splt][indexes])
                ],
                return_tensors="pt",
                padding="longest",
            )
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.embed:
            emb = slic.apply(
                lambda x: self.sbert_model.encode(
                    [
                        (
                            x["text"].text_a
                            if x["text"].text_b == ""
                            else x["text"].text_a + " " + x["text"].text_b
                        )
                    ]
                ),
                axis=1,
        ...
        if self.neigh:
            ...
            for n in slic.neighbours.values[0]:
                neighbourhood.append(
                    n.label / (self.num_classes * (self.num_classes - 1) / 2)
                )
...
class EarlyStopper:
    ...
    def early_stop(self, validation_loss, model):
        # print(f"Min validation loss: {self.min_validation_loss:.4f}")
        if validation_loss < self.min_validation_loss:
            self.min_validation_loss = validation_loss
            self.counter = 0
            torch.save(
                model.state_dict(), "models/" + str(self.timestamp) + "/checkpoint.pt"
            )
...
def train_k_predictor(
    ...
    train_data_loader = DataLoader(
        train_dataset, batch_size=per_device_train_batch_size, shuffle=True
    )
    val_data_loader = DataLoader(
        val_dataset, batch_size=per_device_eval_batch_size, shuffle=False
    )
    test_data_loader = DataLoader(
        test_dataset, batch_size=per_device_eval_batch_size, shuffle=False
    )
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch: {epoch + 1:>2} | Train Loss: {sum(all_train_loss) / len(all_train_loss):.4f} | Val Loss: {sum(all_val_loss) / len(all_val_loss):.4f}"
        )
    ...
    model.load_state_dict(
        torch.load(
            "models/" + str(early_stopper.timestamp) + "/checkpoint.pt",
            weights_only=True,
        )
    )
    all_preds = list()
    ...
    print("Classification Report K predictor model:")
    print(report)
    return model
